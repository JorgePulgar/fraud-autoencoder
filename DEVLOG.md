...
**Concepts needed to understand this entry (plain language):**

- **PR-AUC (Precision-Recall Area Under Curve):** imagine sweeping a detection threshold from "flag everything" down to "flag almost nothing." At each threshold you get a precision (of everything we flagged, what fraction was actually fraud?) and a recall (of all real fraud, what fraction did we catch?). Those two numbers trace a curve. PR-AUC is the area under that curve — higher means you can be precise *and* catch a lot at the same time. On a dataset with 0.17% fraud, this is hard: even one false positive among many true positives tanks precision.
- **Why imbalance hurts PR-AUC specifically:** with 74 fraud cases and 42,648 legit ones in the test set, if your model puts even 50 legit transactions in the "high suspicion" zone alongside the fraud, your precision at that threshold is 74/(74+50) ≈ 60%. PR-AUC averages this across all thresholds, so false positives in the high-score region hurt it severely.
- **Rank-based metric:** PR-AUC only cares about the *order* you sort samples by their score — not the actual score values. If fraud sample A has a higher reconstruction error than legit sample B, that pair hurts your PR-AUC, and it doesn't matter whether the scores are (5.1, 5.0) or (log(5.1), log(5.0)) or (100, 99) — the order is the same, so the PR curve is identical.
- **Why the 147 unusual legit transactions exist:** the autoencoder is trained only on legit transactions, but not all legit transactions look alike. Some real customers have genuinely unusual spending patterns (e.g. a one-off large international transfer). The model reconstructs these poorly — high error — not because they are fraud, but because it has never seen anything like them. From the model's perspective they look anomalous. This is a fundamental limitation of reconstruction-based anomaly detection, not a training bug.

**Root cause:** The 0.70 PR-AUC target was set too optimistically for an unsupervised reconstruction-based method on a severely imbalanced dataset (0.17% fraud). The hard constraint is structural: the test set contains 147 legitimate transactions whose reconstruction errors are higher than some fraud cases (see concept notes above for why). Since PR-AUC is rank-based, no post-processing transform can fix this — log1p(5.1) > log1p(5.0) in the same order as 5.1 > 5.0, so the ranking and thus the PR curve are unchanged. The autoencoder has simply not learned to rank those 147 "unusual legitimate" transactions below all fraud. The Logistic Regression baseline (supervised, full label access) achieved PR-AUC = 0.7928, confirming the features are separable but the unsupervised ceiling is lower.
