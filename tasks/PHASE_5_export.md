...
- [x] `models/autoencoder.pth` exists.

## Tasks

- [x] **Task 5.1** — In `src/export_onnx.py`, implement `export_to_onnx(model_path: str, output_path: str) -> None` that:
...
- [x] **Task 5.2** — In the same file, implement `verify_onnx_export(pytorch_model_path, onnx_model_path) -> None` that:
...
- [x] **Task 5.3** — Add CLI entrypoint and run `python -m src.export_onnx`. Verify:
...
- [x] **Task 5.4** — Update `.gitignore` to track ONNX as ignored for now (we don't want to commit binary artifacts to the main repo — they'll live in the v2 deployment repo later).
  - Commit: `chore(repo): gitignore onnx artifacts`
  - Note: `.gitignore` already had `models/*.onnx` and `*.onnx` from Phase 1 — no change needed.
